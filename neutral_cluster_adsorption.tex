...
\begin{figure}[H]
\centering
\input{neutral_be_distribution_figure}
...
above the triplet, {we also calculated the complexes with a quintet multiplicity to compare
the site-dependent adsorption patterns with different spin state.}
...
span \textcolor{cyan}{from} $-2.353$ to $-1.755$ eV and \textcolor{cyan}{from} $-2.275$ to $-1.547$ eV, respectively.}
...
We investigate whether the coordination environment of the Pt adsorption site is governed solely by the number of neighboring metal atoms or whether their chemical identity also plays a significant role. For this purpose we used Hoppe's
...
them before CO addition avoids mixing the initial site environment with the
...
\begin{figure}[H]
\centering
\input{neutral_be_vs_ptc_figure}
...
\begin{figure}[H]
\centering
\input{neutral_co_frequency_figure}
